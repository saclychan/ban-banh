...
<?
	...
	$pagenum = $_GET["page"];
	$catid=$_REQUEST['cid'];
	$page = "admin.php?go=product_list&cid=".$catid;	
	if($catid==0){
		$sqlPro="select * from product,category where product.CateID=category.CateID order by ProID desc";
	}else{
		$sqlPro="select * from product,category where product.CateID=category.CateID and product.CateID=" .$catid. " order by ProID desc";
	}
...
<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0">
	<tr>
		<td bgcolor="#F0F0F0">
			<span style="font-weight:bold">Trạng thái:</span>
         	<select class="tableboxinput" name="prostatusid" id="prostatusid" onChange="location.href='admin.php?go=product_list&prostatusid='+this.value">
				<option value="0" selected="selected">Xem tất cả sản phẩm</option>
				<option value="1">Các sản phẩm đang bày bán</option>
				<option value="2">Các sản phẩm không bày bán</option>
          	</select>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	      <span style="font-weight:bold">Xem theo loại:</span>
            <select class="tableboxinput" name="select" id="select" onchange="location.href='admin.php?go=product_list&cid='+this.value">
	    ...
	    <td align="right">        	
					...
					<input class="tableboxinput" type="text" name="txtSearch" id="txtSearch" value="<? if($action==ChooseSearch) {echo $txtSearch;} else echo ''?>" onFocus="style.backgroundColor='#F0F0F0'" onBlur="style.backgroundColor='white'"/>
          </td>
          </td></td>
