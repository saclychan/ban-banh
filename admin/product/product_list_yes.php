...
<?
	...
	$pagenum = $_GET["page"];
	$catid=$_REQUEST['cid'];
	$page = "admin.php?go=product_list_yes&cid=".$catid;	
	if($catid==0){
		$sqlPro="select * from product,category where product.CateID=category.CateID and ProStatus=1 order by ProID desc";
	}else{
		$sqlPro="select * from product,category where product.CateID=category.CateID and  ProStatus=1 and product.CateID=" .$catid. " order by ProID desc";
	}
	...
	if($action=="update")
	{
		...
		if(mysql_query($sqlupdate))
		{
			...
			echo("<script>window.location='admin.php?go=product_list_yes';</script>");
		}
	}
	if($action=="del")
	{
		...
		<td bgcolor="#F0F0F0">
			<span style="font-weight:bold">Trạng thái:</span>
         	<select class="tableboxinput" name="prostatusid" id="prostatusid" onChange="location.href='admin.php?go=product_list&prostatusid='+this.value">
				...
				<option value="1" selected="selected">Các sản phẩm đang bày bán</option>
				<option value="2">Các sản phẩm không bày bán</option>
          	</select>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		  <span style="font-weight:bold">Xem theo loại:</span>
            <select class="tableboxinput" name="select" id="select" onchange="location.href='admin.php?go=product_list_yes&cid='+this.value">
